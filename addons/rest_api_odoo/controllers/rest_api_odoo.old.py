...
class RestApi(http.Controller):
    ...
    def clean_records_for_json(self, partner_records):
        _logger.info('about to clean records')
        for record in partner_records:
            for key, value in record.items():
                if isinstance(value, (datetime, date)):
                    _logger.ingo('Just a date or datetime')
                    _logger.info(value)
            ...
            if method == 'GET':
                ...
                if not option.is_get:
                    ...
                    if rec_id != 0:
                        ...
                        self.clean_records_for_json(partner_records)
                        _logger.info(partner_records)
                        data = json.dumps({
